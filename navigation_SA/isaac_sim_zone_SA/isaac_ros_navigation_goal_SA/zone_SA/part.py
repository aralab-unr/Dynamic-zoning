...
class part:
    ...
    def __init__(self,type,ID_num):
        ...
        times = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data/LE','processing_time.csv'), sep=',', header=0, names=['workstation','processing_time'], encoding = 'utf-8')
        self.workstations = times['workstation']
        self.processing_times = times['processing_time']
        variables_set = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data/LE', 'Variables_of_Zone_Warehouse_Phase1.csv'), sep=',', header=0, names=['value'], encoding = 'utf-8')
        ...
        processing_routes = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data/LE','processing_routes_test.csv'), sep=',', header=0, names=['part_type','route','qty'], encoding = 'utf-8')
        ...
        with open(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data','part_status.csv'), 'a') as file:
            line = writer(file)
    ...
    def score(self, robotpos, zone_control):
        Rpos = 'WS' + robotpos
        Pp = 'WS' + self.current_ws
        Pd = 'WS' + self.next_ws
        jobdist = zone_control.shortest_dist(Rpos,Pp) + zone_control.shortest_dist(Pp,Pd)
        score = self.Ca*self.get_age() - self.Cd*((jobdist/self.V)*60) #the bigger the score the higher priority
        return score
    ...
    def add_cycleTime(self):
        with open(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data','cycle_times.csv'), 'a') as file:
    ...
    def write_status(self):
        myrow = []
        with open(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data','part_status.csv'), 'r') as file:
        ...
        with open(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone_SA'), 'isaac_ros_navigation_goal_SA/zone_SA/data','part_status.csv'), 'w') as file:
