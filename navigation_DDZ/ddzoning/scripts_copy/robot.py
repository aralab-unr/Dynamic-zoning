...
from thesis_sim.msg import Row
...
class Robot(Zone_func):
    def __init__(self):
        ...
        carter_pos = rospy.get_param('initial_pose') #current position of the robot
        self.position = [carter_pos[0], carter_pos[1]]
        ...
        super().__init__(self.num_robot, 300)
        self.adj_dist = 300
        ...
        for roboti in range(1,self.num_robot+1): #other robot pos suscribers, suscribe to all other robot positions 
            ns1 = "/robot" + str(roboti) + "/robot_pos"
        ...
        self.window = 15 #defines how long a data point stays recorded (min)
        ...
        self.robot_load = 0
        self.robot_all_zone = []
        self.alarm_time = 1.5 #time till robot declares out of balence (min)
        self.avgc_interval = 50 #time till average consensus (secs)
        self.bal_tolerance = 0.1 #50 #balence tolerance 
        ...
        self.max_episode = 3
        self.max_it = 10
        #self.LF_flag = True #flag to declare if a 
        ...
        self.repeat = 0 #for repeat zone updating (only what to update zone once but somtimes msg isnt sent)
    ...
    def update_zone(self, zone_msg):
        ...
        if self.repeat == 1:
            ...
            rospy.sleep(0.5) #give time for map to recieve new zone
        if self.repeat == 5:
            self.repeat = 0

        #send confirm
        confirmpub = rospy.Publisher(robot_return+"zone_confirm", Bool, queue_size=5)
        rospy.sleep(0.1)
        msg = Bool()
        msg.data = True
        for _ in range(4):
            confirmpub.publish(msg)
            rospy.sleep(0.05)
    ...
    def get_lead_availability(self):
        ...
        for it in range(10):
            ...
            rospy.sleep(0.1)
        if ((all(neigh_avail) == 0) and self.leader_flag == 0):
            return True
        else:
            return False
    ...
    def score(self, part_msg):
        ...
        if self.currentgoal is None:
            self.currentgoal = part_msg.goal1 #set the first part pickup as the first goal

        Rpos = self.currentgoal
        Pp = part_msg.goal1
        Pd = part_msg.goal2

        jobdist = self.get_shortest_dist(Rpos, Pp, self.workstation_dist_mtx) + self.get_shortest_dist(Pp, Pd, self.workstation_dist_mtx)
        score = self.Ca*part_msg.age - self.Cd*((jobdist/self.avgvel)*60) #the bigger the score the higher priority
    ...
    def goto(self, goalws):
        ...
        for point in path:
            ...
            while target_dist > tolerance:
                self.position = self.position + dir*(self.vel*timediff) #move to the next target
    ...
    def monitor(self):
        ...
        while not rospy.is_shutdown():
            ...
            if np.sum(self.recorded_load) > 0 and (curr_time.secs - s2.secs) > 10:
    ...
    def DD_zoning(self):
        ...
        for ep in range(1, self.max_episode+1):
            ...
            #robot neighbors
            self.r_neigh = list(self.neigh_pos.keys())
            print("robot neighbors", self.r_neigh)
            ...
            for it in range(1,self.max_it+1):
                ...
                robotj = random.choice(self.r_neigh) #name "/robot1/"
                ...
                if zonecs_c == zonews: #unable to exhange a tipws
                    print("unable to exchange a tip ws")
                    continue

                #check to see where ws got yonked
                sumsc = [len(z) for z in zonews_c]
                sumzc = sum(sumsc)
                sums = [len(z) for z in zonews]
                sumz = sum(sums)
                if sumz != sumzc:
                    print("missing ws")
                    print(zonews_c)
                ...
                self.pub_ws_assignment()
                rospy.sleep(2)
                for _ in range(len(self.neigh_pos)): #repeat to ensure that every yaml file is correct
                    for neigh in self.neigh_pos.keys():
                        self.new_zone(neigh, zonews_c, zonecs_c, -1) #tell neighbor to update yaml file and to run startup() use current loads          
                ...
                p = math.log1p(100/it)*.3 #curve to accept random probabilities
                r = random.random()
                ...
                if (new_error <= curr_error or r < p) and tssum > 0:
                    ...
                    #self.all_adj_matrix = copy.deepcopy(alladj_c)
                    #rospy.sleep(5)
                    ...
                    for _ in range(len(self.neigh_pos)):
                        for neigh in self.neigh_pos.keys():       
        ...
        print("END OF DD ZONING")
        self.exit_ready = 1 #once all neighbors are ready to exit this robot can exit
        self.DD_done = True
        self.vel = 8.5
        self.leader_flag = 0

    def exchange_ws(self, At, robotj, zonews, zonecs, all_adj_matrix):
    ...
    def calc_error(self):
        ...
        return sum_of_diff/(len(self.neigh_pos)+1) #+1 for roboti
    
    def all_reload(self):
        #tell neighbors to reload parts
        #print("telling neighbor",neighbor, "to reload parts")
        for neigh in self.neigh_pos.keys():
            reloadpub = rospy.Publisher(neigh +"reload_parts",Bool,queue_size=10)
            rospy.sleep(0.1)
            msg2 = Bool()
            msg2.data = True
            reloadpub.publish(msg2)
            rospy.sleep(0.1)
    ...
    def send_reload(self, data):
        ...
        for _ in range(len(to_sendq)):
            ...
            self.pub_part(currentpart)
            rospy.sleep(0.05)
        rospy.sleep(1) #wait to recieve requeued parts
    ...
    def update_neigh_loads(self):
        ...
        for neigh in self.neigh_pos.keys():
            ...
            pub = rospy.Publisher(neigh+"get_load", Zone, queue_size=1)
            rospy.sleep(0.1) #give ros some time to create publisher
            msg = Zone()
            msg.robot = rospy.get_namespace() #give neighbor the return robot
            #msg.stations = all_zonews
            pub.publish(msg)
            #print(rospy.get_namespace()+"n_load")
            #rospy.wait_for_message(rospy.get_namespace()+"n_load", Float32) #wait to get new calculated load from neighbor
            #print("recieved neigh load")
            #return load_msg.data
            rospy.sleep(0.5)
        rospy.sleep(2)
    ...
    def new_zone(self, robotID, zonews, zonecs, j_load):
        ...
        pub = rospy.Publisher(robotID+"adopt_new_zone", Zone, queue_size=10)
        rospy.sleep(0.2)
        msg = Zone()
        msg.robot = rospy.get_namespace()
        ...
        rospy.wait_for_message("zone_confirm",Bool)

    def adjust_neigh_ts(self):
        #print("in adjust neigh ts")
        self.adjust_ts(True)
        for neigh in self.neigh_pos.keys():
            tspub = rospy.Publisher(neigh+"correct_ts",Bool,queue_size=10)
            rospy.sleep(0.1)
            msg = Bool()
            msg.data = True
            tspub.publish(msg)
    ...
    def write_error(self, new_error):
        myrow = []
        with open(os.path.join(rospkg.RosPack().get_path('thesis_sim'), 'scripts/data','error_report.csv'), 'r') as file:
        ...
        with open(os.path.join(rospkg.RosPack().get_path('thesis_sim'), 'scripts/data','error_report.csv'), 'w') as file:
