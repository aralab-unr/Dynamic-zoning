...
class Part:
    ...
    def __init__(self, type,ID_num):
        ...
        times = pd.read_csv(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data/LE','processing_time.csv'), sep=',', header=0, names=['workstation','processing_time'], encoding = 'utf-8')
        ...
        processing_routes = pd.read_csv(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data/LE','processing_routes_test.csv'), sep=',', header=0, names=['part_type','route','qty'], encoding = 'utf-8')
        ...
        for qty in qtylist: 
            ...
            if sum > self.ID:
                break
            index += 1
        print("ID:",self.ID,"index:",index, "type:", self.type)
        ...
        with open(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data','part_status.csv'), 'a') as file:
            line = writer(file)
    ...
    def add_cycleTime(self):
        with open(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data','cycle_times.csv'), 'a') as file:
    ...
    def write_status(self):
        myrow = []
        with open(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data','part_status.csv'), 'r') as file:
        ...
        with open(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data','part_status.csv'), 'w') as file:
