...
class Map(Zone_func):
    def __init__(self):
        ...
        map_pd = pd.read_csv(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data/LE','map.csv'), sep=',', header=0, index_col=0, encoding = 'utf-8')
        ...
        self.station_loc = pd.read_csv(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data/LE','station_location.csv'), sep=',', header=0, names=['x','y'], encoding = 'utf-8')
        ...
        for robotID in range(1,self.num_robot+1):
            self.robot_part_pub.append(rospy.Publisher("/carter"+str(robotID)+"/part_recieve", Goal, queue_size=50))#part publisher
    ...
    def queue_part(self, part, WS):
        ...
        try:
            ...
            print("!!!!error with loading yaml file!!!")
            print("try loading yaml file again")
            print("requeing part")
            rospy.sleep(0.2)
            self.queue_part(part, WS)
            return
        ...
        if (not part.get_transfer()): #case for when the recieving part is not being transfered and being dropped in same zone 
            ...
            try:
                ...
                #somtimes this happens when zonews or robot claim are not properly updated
    ...
    def add_throughput(self):
        with open(os.path.join(rospkg.RosPack().get_path('ddzoning'), 'scripts/data','thoughput.csv'), 'a') as file:
...
def main():
    ...
    print("warming up")
    active_map = Map()
    rospy.sleep(15) #wait until robots start up
