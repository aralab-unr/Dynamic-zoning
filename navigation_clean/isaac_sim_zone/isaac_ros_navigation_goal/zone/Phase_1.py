...
class Phase1:
      ...
      def read_csv(self):
            ...
            #print(var)
            variables_set = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone'), 'isaac_ros_navigation_goal/zone/data/LE', 'Variables_of_Zone_Warehouse_Phase1.csv'), sep=',', header=0, names=['value'], encoding = 'utf-8')
            self.critical_points = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone'), 'isaac_ros_navigation_goal/zone/data/LE','Critical_Points.csv'), sep=',', header=0, names=['x','y'], encoding = 'utf-8')
            ...
            self.adjacency_Mtx_pd = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone'), 'isaac_ros_navigation_goal/zone/data/LE','Adjacency_matrix.csv'), sep=',', header=0, names=list(self.critical_points.index.values), encoding = 'utf-8')
            ...
            self.workstation_loads = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone'), 'isaac_ros_navigation_goal/zone/data/LE','Workstation_Loads.csv'), sep=',', header=0, names=list(self.workstation_loc.index.values), encoding = 'utf-8')
      ...
      def number_of_zones(self):
            ...
            for i in range(self.num_Work_Stations):
                  ...
                  for j in range(self.num_Work_Stations):
                        ...
                        path, distij = self.shortest_dist(nodei,nodej,self.adjacency_Mtx)
      ...
      def draw_map(self):
            ...
            line_ty = np.array(['-', ':', '--', '-.'])
            index = 0
            for row in range(len(self.adjacency_Mtx)):
                  for col in range(len(self.adjacency_Mtx)):
                        if self.adjacency_Mtx[row,col] != 0:
                              x_line = np.array([])
                              y_line = np.array([])
