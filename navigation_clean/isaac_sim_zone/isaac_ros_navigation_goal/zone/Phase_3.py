...
class Phase3(Phase2):
    def __init__(self, zone_ws, crit_seg, allAdjMatrix):
        ...
        workstation_dist_mtx = pd.read_csv(os.path.join(rospkg.RosPack().get_path('isaac_sim_zone'), 'isaac_ros_navigation_goal/zone/data/LE','WS_dist_mtx.csv'), sep=',')
        ...
        for pair_index in range(0,len(self.neighboring_zones)):
            ...
            if TW[0] != None and TW[1] != None:
                ...
                while len(SPC_TW_path) != 0:
                    ...
                    if((A in self.transfer_stations[pair_index]) or (B in self.transfer_stations[pair_index])):
